package Action.adminAction.board;
...
public class ThscSearchAction implements Action {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        //총 게시물 수 구하기
        //처음부터 끝까지 전체의 데이터 갯수
        int timeTableCount = TimeTableDAO.getTimetableList().length;

        //페이징 처리를 위한 객체 생성
        Paging page = new Paging(10, 5); //1페이지당 10개씩, 3페이지

        //총 페이지수를 저장
        page.setTotalCount(timeTableCount);
        ...
        TheaterVO[] ar = TheatherDAO.getThscSearch(page.getBegin(), page.getEnd(), map);
        ...
        request.setAttribute("timeTableCount", timeTableCount); //게시물 토탈 갯수
