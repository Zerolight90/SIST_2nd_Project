...
<div class="dashBody">
  ...
  <div class="admin-container">
    ...
    <table class="member-table">
      ...
      <c:forEach var="vo" items="${requestScope.ar}" varStatus="vs1">
        <tr>
    ...
    <input type="button" value="지점 등록" class="btn writeBoard"
           onclick="javascript:location.href='Controller?type=adminTheaterRegistration'"/>
    ...
    <nav>
      <ol class="pagination">
        ...
        <c:if test="${p.startPage >= p.pagePerBlock}">
          <li class="nav-arrow"><a href="Controller?type=adminTheaterList&cPage=${p.nowPage-p.pagePerBlock}">&lt;</a></li>
        </c:if>
        ...
        <c:forEach begin="${p.startPage}" end="${p.endPage}" varStatus="vs">
          ...
          <%--현재 페이지 외의 버튼들--%>
          <c:if test="${p.nowPage != vs.index}">
            <li><a href="Controller?type=adminTheaterList&cPage=${vs.index}">${vs.index}</a></li>
          </c:if>
        </c:forEach>


        <c:if test="${p.endPage < p.totalPage}">
          <li><a href="Controller?type=adminTheaterList&cPage=${p.nowPage+p.pagePerBlock}">&gt;</a></li> <%--&gt; :: >>--%>
        </c:if>
        <c:if test="${p.endPage >= p.totalPage}">
          <li class="nav-arrow disable">&gt;</li>
        </c:if>
...
<script>
  $(function () {
    $(".btn-search").on('click', function () {
      ...
      $.ajax({
        url: "Controller?type=adminTheaterSearch",
        type: "GET",
        data: formdata,
        dataType: "html",
