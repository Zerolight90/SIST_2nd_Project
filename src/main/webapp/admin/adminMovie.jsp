  ...
  <div class="admin-container">
    ...
    <div class="control-bar">
      <div class="total-count">
        전체 <strong>${requestScope.movieCount}</strong>건
      </div>
    ...
    <nav>
      <ol class="pagination">
        ...
        <c:if test="${p.startPage >= p.pagePerBlock}">
          <li class="nav-arrow"><a href="Controller?type=adminMoviePaging&cPage=${p.nowPage-p.pagePerBlock}">&lt;</a></li>
        </c:if>
        ...
        <c:forEach begin="${p.startPage}" end="${p.endPage}" varStatus="vs">
          ...
          <%--현재 페이지 외의 버튼들--%>
          <c:if test="${p.nowPage != vs.index}">
            <li><a href="Controller?type=adminMoviePaging&cPage=${vs.index}">${vs.index}</a></li>
          </c:if>
        </c:forEach>


        <c:if test="${p.endPage < p.totalPage}">
          <li><a href="Controller?type=adminMoviePaging&cPage=${p.nowPage+p.pagePerBlock}">&gt;</a></li> <%--&gt; :: >>--%>
        </c:if>
        <c:if test="${p.endPage >= p.totalPage}">
          <li class="nav-arrow disable">&gt;</li>
        </c:if>
