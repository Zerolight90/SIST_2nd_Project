<%@ page contentType="text/html;charset=UTF-8" language="java" %>

<nav>

  <div class="dashHead bold">
    <div style="display: inline-block; justify-content: space-between; align-items: center"><p style="margin-left: 10px">admin 관리자님</p></div>
    <div style="display: inline-block; float: right; padding-top: 13px; padding-right: 10px">
      <a href="">SIST</a>
      <a href="">로그아웃</a>
    </div>
  </div>
  <div class="dashLeft" style="width: 250px; display: inline-block">
    <div class="leftDiv"><p class="bold">대시보드</p></div>
    <div class="leftDiv">
      <p class="bold">사용자 관리</p>
      <div class="leftDivInner"><a href="">회원 정보 관리</a></div>
      <div class="leftDivInner"><a href="">비회원 정보 관리</a></div>
    </div>
    <div class="leftDiv">
      <p class="bold">영화 관리</p>
      <div class="leftDivInner"><a href="">영화 정보 관리</a></div>
      <div class="leftDivInner"><a href="">영화 상영 관리</a></div>
    </div>
    <div class="leftDiv">
      <p class="bold">극장 관리</p>
      <div class="leftDivInner"><a href="">극장 / 상영관 관리</a></div>
    </div>
    <div class="leftDiv">
      <p class="bold">상품 관리</p>
      <div class="leftDivInner"><a href="">상품 정보 관리</a></div>
    </div>
    <div class="leftDiv">
      <p class="bold">결제 관리</p>
      <div class="leftDivInner"><a href="">결제 내역 관리</a></div>
    </div>
    <div class="leftDiv">
      <p class="bold">게시판 관리</p>
      <div class="leftDivInner"><a href="">공지 / 이벤트 관리</a></div>
    </div>
    <div class="leftDiv">
      <p class="bold">쿠폰 관리</p>
      <div class="leftDivInner"><a href="">쿠폰 정보 관리</a></div>
    </div>
    <div class="leftDiv">
      <p class="bold">시스템 관리</p>
      <div class="leftDivInner"><a href="">관리자 권한 관리</a></div>
      <div class="leftDivInner"><a href="">관리자 / 사용자 로그 관리</a></div>
    </div>
  </div>

</nav>